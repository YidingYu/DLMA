...
def my_plot(file1, file2, q):
	...
	agent1_optimal = np.ones(max_iter)* ((1-q)/2)
	agent2_optimal = np.ones(max_iter)* (q/2)
	...
	print('---------------')
	print('agent', np.mean(throughput_agent1[0][-50000:]))
	print('aloha', np.mean(throughput_agent2[0][-50000:]))

for i in range(1, 5):
	plt.figure(i)
	my_plot('rewards/agent_len1e5_M20_h6_q0.4_%d.txt' % i,
		    'rewards/aloha_len1e5_M20_h6_q0.4_%d.txt' % i, q=0.4)
plt.show()
