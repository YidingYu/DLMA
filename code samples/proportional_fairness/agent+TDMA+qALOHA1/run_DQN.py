...
def main(max_iter):
    ...
    for i in range(max_iter): 
        ...
        if state_length<6:
            next_state = np.concatenate([agent_action, observation_, agent_reward, aloha_reward, tdma_reward])
        else:
            next_state = np.concatenate([state[5:], [agent_action, observation_, agent_reward, aloha_reward, tdma_reward]])

        dqn_agent.store_transition(state, agent_action, agent_reward, aloha_reward, tdma_reward, next_state)
        if i > 200:
            dqn_agent.learn()       # internally iterates default (prediction) model
        state = next_state

    with open('rewards/agent_len5e5_M20_h6_q0.1_t10-3_1.txt', 'w') as my_agent:
        for i in agent_reward_list:
            my_agent.write(str(i) + '   ')
    with open('rewards/aloha_len5e5_M20_h6_q0.1_t10-3_1.txt', 'w') as my_aloha:
        for i in aloha_reward_list:
            my_aloha.write(str(i) + '   ') 
    with open('rewards/tdma_len5e5_M20_h6_q0.1_t10-3_1.txt', 'w') as my_tdma:
        for i in tdma_reward_list:
            my_tdma.write(str(i) + '   ') 
    # save model 
    # dqn_agent.save_model("models/model_len5e5_M20_h6_q0.1_t10-3_1.h5")  
    # print the results
    print('-----------------------------')
...
if __name__ == "__main__":
    env = ENVIRONMENT(state_size=100, 
                      attempt_prob=0.1
                      )
