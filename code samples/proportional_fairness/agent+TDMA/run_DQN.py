...
def main(max_iter):
    ...
    for i in range(max_iter): 
        ...
        if len(state)<5:
            next_state = np.concatenate([agent_action, observation_, agent_reward, tdma_reward])
        else:
            next_state = np.concatenate([state[4:], [agent_action, observation_, agent_reward, tdma_reward]])

        dqn_agent.store_transition(state, agent_action, agent_reward, tdma_reward, next_state)
        if i > 200:
            dqn_agent.learn()       # internally iterates default (prediction) model
        state = next_state

    with open('rewards/agent_len1e5_M20_h6_t10-6_1.txt', 'w') as my_agent:
        for i in agent_reward_list:
            my_agent.write(str(i) + '   ')
    with open('rewards/tdma_len1e5_M20_h6_t10-6_1.txt', 'w') as my_tdma:
