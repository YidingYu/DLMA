...
def main(max_iter):
    ...
    with open('rewards/agent_len5e4_M20_h1_t10-3_1.txt', 'w') as my_agent:
        for i in agent_reward_list:
            my_agent.write(str(i) + '   ')
    with open('rewards/tdma_len5e4_M20_h1_t10-3_1.txt', 'w') as my_tdma:
        for i in tdma_reward_list:
            my_tdma.write(str(i) + '   ') 

 
    # save model 
    # dqn_agent.save_model("models/model_len5e4_M20_h1_t10-3_1.h5")  
